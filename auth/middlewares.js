...
const check = roles => {
  return async function (req, res, next) {
    ...
    const session = await Sessions.findOne({ _id: token, active: true }).populate('user').lean().exec();
    if (!session) {
      if (req.cookies.authorization) res.clearCookie('authorization');
...
const generate_magiclink = (appUrl, mailsFrom) => {
  ...
  return async function (req, res, next) {
    ...
    const user = await process.Users.findOne({ email: req.body.email }).exec();
    if (!user || !user._id)
...
const new_pass_reset = (appUrl, mailsFrom) => {
  ...
  return async function (req, res, next) {
    ...
    await process.Users.updateOne({ email: req.body.email }, { $push: { resetTokens: token } }).exec();
    const url = `${appUrl}/auth/reset-password/${token}`;
    console.log(req.body);
    await email.send(mailsFrom, req.body.email, 'pass_reset', { url, firstname: user.firstname });
    return res.status(200).json({ success: true, message: 'mail is on its way, check your inbox!' });
  };
};
...
const pass_reset = () => {
  return async function (req, res, next) {
    if (!req.body.token || !req.body.password)
      return res.status(400).json({ success: false, error: 'not all fields are set' });
    const user = await process.Users.findOne({ resetTokens: req.body.token }).exec();
    if (!user)
      return res.status(403).json({ success: false, error: 'invalid token... dont hack us, join us!' });
    const password = await bcrypt.hash(req.body.password, 8);
    await process.Users.updateOne({ resetTokens: req.body.token }, { password, resetTokens: [] }).exec();
    return res.status(200).json({ success: true });
  };
};
